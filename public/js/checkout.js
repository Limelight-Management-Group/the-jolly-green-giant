...
$(document).ready(function(){
    ...
    $(".productItem").click(function(e){
        ...
        let a = (index+1)
            ...
            holderHTML +=  
            `<tr><td><input type="text" name="quantity_"avalue="${value.qty}"></td>
            <td><input type="hidden" name='item_name_'avalue="${(value.name)}"> ${(value.name)}</td> 
            <td class="text-xs-right"> $ ${formatMoney(value.price)} <input type="hidden" name="amount_"avalue="
            ${formatMoney(value.price)}"></td>
            <td class="text-xs-right"> $ ${formatMoney(subTotal)} </td></tr>
            <br>`
